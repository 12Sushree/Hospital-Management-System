...
function NurseMedication() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://hmsmern.onrender.com/user/get-users"
        );
  ...
  const handleAddMedication = async (e) => {
    e.preventDefault();
    await axios
      .post(`http://localhost:3000/user/add-medications/${changePatient}`, {
        name,
        frequency,
        dosage,
      })
            ...
            <div>
              ...
              >
                {patients.map((value, index) => {
                  return <option value={value.email}>{value.userName}</option>;
                })}
              </select>
            </div>
            <div>
